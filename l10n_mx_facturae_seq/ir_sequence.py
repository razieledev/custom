...
class ir_sequence(osv.Model):
    ...
    def copy(self, cr, uid, id, default={}, context=None, done_list=[], local=False):
        if isinstance(id, (int,long)): ids = [id]
        if context is None:
            context = {}
        for seq in self.browse(cr, uid, ids):
            if not seq.company_id and seq.company_id.mexican_localization:
    ...
    def _get_current_approval(self, cr, uid, ids, field_names=None, arg=False, context=None):
        ...
        for sequence in self.browse(cr, uid, ids, context=context):
            ...
            print "all data--->", sequence.id, number_work, sequence.company_id.id
