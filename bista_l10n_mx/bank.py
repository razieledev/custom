...
class res_bank(osv.Model):
    ...
    _order = 'sat_code'
    _columns = {
        'sat_code': fields.char('Código SAT', size=3),
        'full_name': fields.char('Nombre Completo', size=128),
    }


class account_account_journal(osv.Model):
    _name = 'account.journal'
    _inherit = 'account.journal'
    _columns = {
        'res_partner_bank_id': fields.one2many(
            'res.partner.bank',
            'journal_id',
            'Cuenta bancaria',
            help='Cuenta bancaria para identificar los registros contables'
        ),
    }
