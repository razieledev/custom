...
from openerp.addons.account.report import account_aged_partner_balance
#from openerp 


class aged_trial_report_inhe1(account_aged_partner_balance.aged_trial_report):


    def __init__(self, cr, uid, name, context):
        super(aged_trial_report_inhe1, self).__init__(cr, uid, name, context=context)
#        self.total_account = []
        self.localcontext.update({
#            'time': time,
#            'get_lines_with_out_partner': self._get_lines_with_out_partner,
            'get_lines_mxn': self._get_lines_mxn,
            'get_lines_with_out_partner_mxn': self._get_lines_with_out_partner_mxn,
...
#    this function is used for need partner aged report in usd
    def _get_lines(self, form):
        ''' this function returning usd journal items base on account currency is null
            (account_account.currency_id is null
            adding this to exisiting query
        '''
        res = []
        ...
        totals = {}
        self.cr.execute('SELECT l.partner_id, SUM(l.amount_currency) \
        ...
        for i in range(5):
            ...
            self.cr.execute('''SELECT l.partner_id, SUM(l.amount_currency), l.reconcile_partial_id
                    FROM account_move_line AS l, account_account, account_move am
                    WHERE (l.account_id = account_account.id) AND (l.move_id=am.id)
                        AND (am.state IN %s)
                        AND (account_account.type IN %s)
    ...
    def _get_lines_mxn(self, form):
        self.total_account = []
        ''' this function returning mxn journal items base on account 2nd currency
            (account_account.currency_id is null
            adding this to exisiting query
        '''
        res = []
    ...
    def _get_lines_with_out_partner_mxn(self, form):
        self.total_account = []
        ''' this function returning mxn journal items base on account 2nd currency
            (account_account.currency_id is null
            adding this to exisiting query
...
class report_agedpartnerbalance_inh(account_aged_partner_balance.report_agedpartnerbalance):
    _name = 'report.account.report_agedpartnerbalance'
    _inherit = 'report.abstract_report'
    _template = 'account.report_agedpartnerbalance'
    _wrapped_report_class = aged_trial_report_inhe1

report_agedpartnerbalance_inh()
